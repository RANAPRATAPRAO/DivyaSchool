      ...
      <div className="w-full max-w-6xl py-14 flex flex-col items-center">
        ...
        <div className="flex flex-col lg:flex-row w-full gap-16 items-center justify-between">
          ...
          <div className="flex-1 w-full max-w-xl flex flex-col gap-10">
            ...
            {/* Past Results Chart (Bar graph) - RESPONSIVE */}
            <motion.div
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.35, duration: 0.7 }}
              className="bg-blue-50 dark:bg-gray-900 rounded-2xl shadow-lg p-4 sm:p-8"
            >
              ...
              <div className="w-full overflow-x-auto">
                <div className="flex gap-2 sm:gap-4 justify-center items-end h-[120px] min-w-[390px] sm:min-w-0 px-2">
                  {pastResults.map(({ month, value }, i) => (
                    <motion.div
                      ...
                      className="flex flex-col justify-end items-center min-w-[36px] sm:min-w-[48px]"
                    >
                      <div
                        className="w-6 sm:w-12 rounded-xl bg-gradient-to-b from-blue-400 via-emerald-400 to-yellow-400 dark:from-cyan-700 dark:via-cyan-400 dark:to-cyan-200 shadow-md mb-2 transition-all"
                        style={{ height: `${30 + value * 1.2}px` }}
                      ></div>
                      <span className="text-center text-blue-900 dark:text-cyan-200 mt-1 font-semibold text-xs sm:text-lg whitespace-nowrap">{month}</span>
                      <span className="text-xs text-gray-500 dark:text-cyan-400">{value}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
